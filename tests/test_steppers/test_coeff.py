...
from qmint.diffops.odes import Dahlquist, Lorenz, ProtheroRobinson
...
def testLinearCoeffStepperDahlquist(scheme, tEnd, nSteps, lam):
    diffOp = Dahlquist(lam=lam)
    stepper = CoeffStepper(diffOp=diffOp, tEnd=tEnd, nSteps=nSteps)

    qGen = Q_GENERATORS[scheme].getInstance()

    uRef = qGen.solveDahlquist(lam, 1, tEnd=tEnd, nSteps=nSteps)

    uNum = stepper.run(Q=qGen.Q, weights=qGen.weights)
    uNum = uNum[:, 0] + 1j*uNum[:, 1]

    assert np.allclose(uNum, uRef), \
        "generic CoeffStepper does not match reference stepper for Dahlquist"

    if scheme.startswith("ARK443"):
        uNum = stepper.run(Q=qGen.Q, weights=None)
        uNum = uNum[:, 0] + 1j*uNum[:, 1]

        assert np.allclose(uNum, uRef), \
            "generic CoeffStepper without weights does not match reference stepper for Dahlquist"
...
def testLinearCoeffStepperDahlquistSDC(
    ...
    for weights in [coll.weights, None]:
        ...
        assert np.allclose(uNum, uRef), \
            f"generic CoeffStepper SDC {details} does not match reference stepper for Dahlquist"
...
def uRefLorenz():
    ...
    uRef = CoeffStepper(diffOp, tEnd=tEnd, nSteps=10000).run(
        qGenRef.Q, qGenRef.weights)
    return {"tEnd": tEnd, "sol": uRef, "diffOp": diffOp}


@pytest.mark.parametrize("scheme", ["BE", "FE", "TRAP",  "RK4", "DIRK43"])
def testLinearCoeffStepperLorenz(scheme, uRefLorenz):
    ...
    for nSteps in nStepsVals:
        stepper = CoeffStepper(diffOp, tEnd=tEnd, nSteps=nSteps)
        uNum = stepper.run(qGen.Q, qGen.weights)
...
def testLinearCoeffStepperLorenzSDC(scheme, nNodes, nSweeps, quadType, uRefLorenz):
    diffOp = Lorenz()
    uRef = uRefLorenz["sol"]
    tEnd = uRefLorenz["tEnd"]
    ...
    for nSteps in nStepsVals:
        stepper = CoeffStepper(diffOp, tEnd=tEnd, nSteps=nSteps)
        uNum = stepper.runSDC(nSweeps, coll.Q, coll.weights, QDelta)
        err.append(np.linalg.norm(uNum[-1] - uRef[-1]))
...
def testLinearCoeffStepperProtheroRobinson(scheme, uRefProtheroRobinson):
...
def testLinearCoeffStepperProtheroRobinsonSDC(scheme, nNodes, nSweeps, quadType, uRefProtheroRobinson):
    diffOp = uRefProtheroRobinson["diffOp"]
    uRef = uRefProtheroRobinson["sol"]
    tEnd = uRefProtheroRobinson["tEnd"]
